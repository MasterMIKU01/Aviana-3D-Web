...
export function normalizeModelDefinition(modelValue) {
  ...
  if (typeof modelValue === 'object') {
    const kind = modelValue.format || modelValue.kind || modelValue.type || 'bundle';

    if (kind === 'obj' && modelValue.path) {
      let basePath = modelValue.path;
      if (!basePath.endsWith('/')) basePath += '/';
      const folderName = basePath.split('/').filter(Boolean).pop() || '';
      
      return {
        kind: 'obj',
        url: modelValue.obj || `${basePath}${folderName}.obj`,
        obj: modelValue.obj || `${basePath}${folderName}.obj`,
        mtl: modelValue.mtl || `${basePath}${folderName}.mtl`,
        texturePath: modelValue.texturePath || basePath
      };
    }
